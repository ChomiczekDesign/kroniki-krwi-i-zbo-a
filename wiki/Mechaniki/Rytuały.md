# Rytuały

**Rytuały** są formą magii wymagającą przygotownia i długiego czasu. W przeciwieństwie do zwykłych zaklęć nie służą do wywoływania natychmiastowych efektów, lecz pozwalają oddziaływać na **większą skalę**, przez **dłuższy czas** albo ingerować w magię w sposób zbyt złożony, by osiągnąć go pojedynczym czarem.

Rytuał może trwać od godziny do wielu godzin lub dni. Często wymaga przygotowania odpowiedniego miejsca, komponentów, instrumentów rytualnych oraz udziału kilku osób.

Rytuały są osobną formą używania magii i **nie korzystają bezpośrednio z zasad budowania zaklęć**. Każdy rytuał posiada własny efekt, wymagania i parametry.

---

## Elementy rytuału
Każdy rytuał określa:
- **Efekt** — co rytuał ma osiągnąć.
- **Trudność** — bazową trudność rzutu rytualnego.
- **Moc Rytuału** — ilość energii przepływającej przez rytuał. Określa kość kosztu oraz ryzyko [[Backlash|Backlashu]], ale **nie określa jego trudności**.
- **Bazowy Czas** — ile czasu trwa rytuał przed uwzględnieniem wyniku rzutu.
- **Wymagania** — warunki niezbędne do rozpoczęcia rytuału, np. miejsce, pora, komponenty lub określone osoby.
- **Instrumenty Rytualne** — jeśli są wymagane. Mogą modyfikować trudność, zabezpieczać uczestników albo zmieniać działanie rytuału.
- **Zasady Specjalne** — wszystkie reguły charakterystyczne dla danego rytuału.

# Skrócona procedura rytuału
- 1.**Spełnij Wymagania** rytuału i przygotuj Instrumenty.
- 2.**Wybierz Prowadzącego** i wszystkich uczestników.
- 3.Prowadzący wykonuje **test Old Lore** przeciwko Trudności Rytuału.
- 4.Sprawdź:
    - sukces lub porażkę,
    - skrócenie czasu przez #Success,
    - możliwość wystąpienia [[Backlash|Backlashu]].
- 5.Jeśli wystąpił [[Backlash]] — rzuć **d8 + modyfikatory** na tabelę Backlashu.
- 6.Rozpocznij odliczanie pozostałego **Czasu Rytuału**.
- 7.Za każdą godzinę:
    - wylosuj uczestnika,
    - wykonaj rzut [[Koszt Rytuału|Kosztu Rytuału]]
- 8.Uczestnicy mogą opuszczać rytuał, ale **nikt nie może dołączyć po jego rozpoczęciu**.
- 9.Po upływie wymaganego czasu rytuał osiąga swój efekt, o ile nie został wcześniej przerwany.

# Przeprowadzanie Rytuału

## 1. Przygotowanie
- Przed rozpoczęciem rytuału muszą zostać spełnione wszystkie jego **Wymagania**.
- Dotyczy to między innymi:
	- przygotowania miejsca,
	- zgromadzenia komponentów (jeśli sa potrzebne)
	- przygotowania instrumentów rytualnych (jeśli sa potrzebne),
	- zebrania uczestników.
- Po rozpoczęciu rytuału **nowe osoby nie mogą już do niego dołączyć**.
## 2. Prowadzący i uczestnicy
- Jedna osoba zostaje **Prowadzącym Rytuał** i wykonuje rzut rytualny.
- W rytuale może uczestniczyć dowolna liczba dodatkowych osób.
- Aby normalnie uczestniczyć w rytuale, postać musi władać magią, **lub** posiadać co najmniej **1 rangę Old Lore**.
	- Sama obecność dodatkowych uczestników nie dodaje #Boost do rzutu rytualnego.
	- Większa liczba uczestników pozwala jednak rozłożyć pomiędzy więcej osób obciążenie związane z utrzymywaniem rytuału.
	- **Grupa Minionów** jest traktowana jak jeden uczestnik rytuału, chyba że opis konkretnego rytuału mówi inaczej.
![[Ofiary Rytualne]]

## 3. Rzut rytualny
- Prowadzący wykonuje test **Old Lore** przeciwko Trudności Rytuału.
- Trudność może zostać zmodyfikowana przez:
	- instrumenty rytualne,
	- użyte komponenty,
	- stan miejsca,
	- wcześniejsze przygotowania,
	- inne warunki określone przez dany rytuał.
- **Moc Rytuału nie wpływa bezpośrednio na trudność rzutu rytualnego**
- Rytuał wymaga #Success , aby osiągnąć zamierzony efekt.
	- **Każdy #Success ponad pierwszy skraca czas rytuału o 1 godzinę** (do minimum 1 h)
- Jeżeli test zakończy się **porażką**, rytuał rozpada się w 1 godzinie trwania i jest traktowany jako nieudany.
	- #Threat i #Despair nadal są rozpatrywane normalnie i mogą spowodować **Backlash** lub inne efekty
## 4.Utrzymywanie rytuału
- Po prawidłowym rozpoczęciu rytuału uczestnicy muszą podtrzymywać go przez cały pozostały czas.
- Każda godzina rytuału generuje Strain zależny od jego **Mocy**.
- Co godzinę należy losowo wybrać jednego uczestnika rytuału. Wybrany uczestnik przyjmuje pełny godzinowy koszt Strainu.
- Przykład:
	- Rytuał o **Znacznej Mocy** generuje 2 Strain na godzinę.  
	- Jeżeli uczestniczą w nim cztery osoby, co godzinę losowana jest jedna z nich i przyjmuje 2 Strain.
- Liczba uczestników **nie zmniejsza ilości generowanego Strainu** — zwiększa jedynie liczbę osób, pomiędzy które może zostać rozłożone obciążenie.
### Opuszczenie rytuału
- Uczestnik może **dobrowolnie opuścić rytuał w dowolnym momencie**.
- Po opuszczeniu rytuału:
	- nie uczestniczy już w jego dalszym utrzymywaniu,
	- nie jest uwzględniany przy kolejnych efektach Kosztu Rytuału
	- **nie może ponownie dołączyć do tego samego rytuału**.
 - Uczestnik opuszcza rytuał w momencie, w którym znajdzie się poza jego kręgiem.
   - Jeżeli efekt mówi, że uczestnik nie może opuścić rytuału, magia uniemożliwia mu przekroczenie granicy Kręgu. Dokładna forma tego ograniczenia zależy od natury rytuału (np. niewidzialna bariera, gwałtowny wiatr, magiczne więzy)
   - Jeżeli efekt powoduje Wyrwanie z Rytuału, sam rytuał gwałtownie usuwa uczestnika poza swój Krąg. Może oznaczać to fizyczne odrzucenie, podmuch energii, szarpnięcie magiczną siłą lub inne gwałtowne zjawisko właściwe dla danego rytuału.
# Instrumenty Rytualne
- Niektóre rytuały wymagają lub mogą mieć opcjonalne **Instrumenty Rytualne**
- Instrumenty są magicznymi przedmiotami służącymi do kontrolowania określonego elementu rytuału. Mogą między innymi:
	- kierować przepływem magii,
    - stabilizować rytuał,
    - przejmować przeciążenia,
    - definiować lub utrzymywać wzorzec efektu,
    - zmniejszać Trudność,
    - zmieniać kości znajdujące się w puli,
    - chronić uczestników przed Backlashem.
- Instrument może być prostym przedmiotem symbolicznym albo skomplikowaną konstrukcją magiczną.
- Jego dokładne działanie określa opis rytuału oraz — jeśli został wykonany przez postać — **wynik jego stworzenia**.
- Instrumenty nie muszą być uniwersalne. Przedmiot przygotowany do konkretnego rytuału może nie mieć żadnego zastosowania przy innym.

![[Moc Rytuału]]

![[Koszt Rytuału]]

![[Backlash]]

![[Rytuały Chaotyczne]]


